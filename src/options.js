...
function normalizeParsers(parsers) {
  ...
  return parsers.map((p, i) => {
    if (typeof p === "string") {
      return { name: safeHostname(p) || ("接口" + (i + 1)), url: p };
    }
    const url = typeof p?.url === "string" ? p.url : "";
    if (!url) return null;
    return { name: p.name || safeHostname(url) || ("接口" + (i + 1)), url };
  }).filter(Boolean);
}
...
function rowTpl(i, p) {
  return `<tr data-i="${i}"><td>${i + 1}</td><td><input class="nm" type="text" value="${escapeAttr(p.name || "")}"/></td><td><input class="ux" type="text" value="${escapeAttr(p.url || "")}"/></td><td><button class="up ghost">上移</button> <button class="down ghost">下移</button> <button class="del danger">删除</button></td></tr>`;
}
